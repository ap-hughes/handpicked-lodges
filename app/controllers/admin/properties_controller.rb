class Admin::PropertiesController < Admin::AdminController
  ...
  def property_params
    params.require(:property).permit(:code, :in_aviemore, :in_country, :enabled, :name, :sleeps, :headline, :description, :min_daily_price, :bedrooms, :bathrooms,
      :wood_stove, :hot_tub, :pet_friendly, :sauna, :games_room, :features, :we_love, :guests_love, :meta_title, :meta_description, :latitude, :longitude, :hero_image, :main_image, :floorplan, photos_attributes: [:id, :image, :key], features_attributes: [:id, :name, :key])
  end
end
